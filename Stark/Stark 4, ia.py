...
def  imprimir_ficha_heroe(personaje:dict):
    ...
    encabezado_de_señas = generar_encabezado("señas particulares")
    ojos = (personaje["color_ojos"]) 
    pelo = (personaje["color_pelo"]) 
    respuesta = ('''{0}NOMBRE DEL HEROE: {1} ({2})\nID SECRETA: {3}\nCONSULTORA: {4}\nCÓDIGO DE HÉROE: {5}
{6}ALTURA: {7}\nFUERZA: {8} \nPESO: {9}\n{10}\nCOLOR DE PELO: {11}\nCOLOR DE OJOS: {12} '''.format(encabezado_principal,nombre,iniciales,identidad,consultora,codigo,encabezado_fisico,altura,peso,fuerza,encabezado_de_señas,ojos,pelo))
    return respuesta
...
def stark_navegar_fichas(lista_personajes:list):
    ...
    while flag_navegar:
        opcion = input("Ingresar la opcion que usted quiera: ([ 1 ] Ir a la izquierda, [ 2 ] Ir a la derecha, [ 3 ] Salir): ")
        if opcion == "1":
            if indice == "0":
                indice = 23
            else: 
